...
def path_is_available_for_obj(f, path, obj_type):
    """
    Check if a path is available for the creation of an object of a certain
    type.
    """
    ...
    if not isinstance(path, str):
        raise TypeError('String expected.')
    if not ((obj_type == h5py.Group) or (obj_type == h5py.Dataset) or
            (obj_type == h5py.NamedDatatype)):
        raise TypeError('h5py object type expected.')

    if path == '' or path[-1] == '/':  # path is empty or has a trailing slash
        return False
    ...
    is_absolute = False
    if path[0] == '/':  # get rid of a leading slash
        is_absolute = True
        path = path[1:]
    ...
    for i in range(len(a)):
        ppath += a[i]
        if ppath not in f:  # unused -> all set
            return True
        else:  # path is in use
